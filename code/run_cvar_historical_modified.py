...
def run_backtest(returns, regimes=None):
    ...
    var_series = pd.Series(var_values, index=result_dates, name="var_estimate")
    ...
    return returns_series, turnover_series, cvar_series, var_series, weights_df, regime_series
...
if __name__ == "__main__":
    ...
    r, turnover, cvar_est, var_est, weights, regime_at_decision = run_backtest(returns, regimes)
    ...
    print(f"  avg_VaR (eta)   {var_est.mean():.4f}  (mean of the eta threshold at each rebalance)")
    ...
    var_est.to_csv(OUTPUT_DIR / "historical_cvar_var.csv")
    if regime_at_decision is not None:
        regime_at_decision.to_csv(OUTPUT_DIR / "historical_cvar_regime_at_decision.csv")
    print(f"\nSaved returns, weights, turnover, VaR, and regime-at-decision to {OUTPUT_DIR}/")
